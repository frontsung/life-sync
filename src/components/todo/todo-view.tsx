...
import { addTodo, toggleTodo, deleteTodo, unlinkTodo, updateTodo, syncTodo } from '@/app/actions';
...
export function TodoView({ todos }: TodoViewProps) {
  ...
  const handleEditStart = (todo: Todo) => {
    ...
    setSyncingId(null); // Close other modes
  };
  ...
  const handleLateSyncStart = (todo: Todo) => {
    if (todo.syncedEventId) return; // Already synced
    setSyncingId(todo.id);
    setEditingId(null); // Close other modes
  };

  const handleLateSyncSave = async (id: string) => {
    startTransition(async () => {
      await syncTodo(id, syncColor);
      setSyncingId(null);
    });
  };
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
             <div className="flex items-center gap-6">
                ...
                {/* Color Selection - Only visible when sync is checked */}
                <div className={cn("transition-opacity duration-200 flex items-center gap-2", isSyncChecked ? "opacity-100" : "opacity-0 pointer-events-none")}>
              ...
              <div className="flex items-center gap-2 sm:ml-4 justify-end">
                ...
                {todo.syncedEventId ? (
                   <div className="flex items-center gap-1">
                     ...
                     <button
                        onClick={() => startTransition(() => unlinkTodo(todo.id))}
